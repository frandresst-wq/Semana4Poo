...
class Estudiante(Persona):
    ...
    def mostrar_informacion(self):
        print(
            f"Nombre: {self._nombre}, RUT: {self._rut}, Carrera: {self._carrera}")
...
class Docente(Persona):
    ...
    def mostrar_informacion(self):
        print(
            f"[Docente] {self._nombre} | RUT: {self._rut} | Especialidad: {self._especialidad}")
...
class Curso:
    ...
    def inscribir_estudiante_curso(self, estudiante: Estudiante):
        if estudiante not in self._estudiantes_inscritos:
            ...
            print(
                f"Estudiante {estudiante.obtener_nombre()} inscrito en el curso de {self._asignatura.obtener_nombre_asignatura()}.")
            return True
        else:
            print(
                f"El estudiante {estudiante.obtener_nombre()} ya está inscrito en este curso.")
            return False
    ...
    def asignar_calificacion(self, estudiante: Estudiante, calificacion: float):
        ...
        if nombre_est in self._calificaciones:
            if 1.0 <= calificacion <= 7.0:
                ...
                print(
                    f"Calificación {calificacion} asignada a {nombre_est} en {self._asignatura.obtener_nombre_asignatura()}.")
            else:
                print("Calificación inválida. Debe estar entre 1.0 y 7.0.")
        else:
            print(
                f"El estudiante {nombre_est} no está inscrito en este curso.")
    ...
    def determinar_aprobacion(self, estudiante: Estudiante):
        ...
        if nombre_est in self._calificaciones:
            ...
            if calificaciones:
                ...
                print(
                    f"El estudiante {nombre_est} ha {estado} con un promedio de {promedio:.2f}.")
                return aprobado
            else:
                print(
                    f"No hay calificaciones registradas para el estudiante {nombre_est}.")
                return False
        else:
            print(
                f"El estudiante {nombre_est} no está inscrito en este curso.")
...
if __name__ == "__main__":
    ...
    asignatura_poo = Asignatura(
        "IT1617A-711-CH2026-T1", "Programación Orientada a Objetos")
    docente_pablo = Docente("PABLO LASTRA CID", "18.144.701-K", "Astronomía")
    est_franco = Estudiante("Franco", "19.155.619-4",
    ...
    print(
        f"Arancel final a pagar por {est_franco.obtener_nombre()}: ${est_franco.calcular_arancel_final():.2f}")
